...
require_once '../system/connect.php'; // Kết nối tới database
...
if (isset($_SESSION['role']) && intval($_SESSION['role']) !== 3) {
...
// Lấy danh sách GradeID và GradeName mà học sinh thuộc lớp
$sqlGrade = "
    SELECT DISTINCT 
        classes.GradeID, grade.GradeName, classes.id as ClassID 
    FROM 
        classes 
    INNER JOIN 
        grade ON classes.GradeID = grade.id
    INNER JOIN
        students ON classes.id = students.ClassID
    WHERE 
                ...
                <div class="table-responsive">
                    <table class="table table-striped table-bordered">
                        ...
                        <tbody>
                            ...
                            <?php else : ?>
                                <tr>
                                    <td colspan="4" class="text-center">Không có thời khóa biểu nào.</td>
                                </tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
